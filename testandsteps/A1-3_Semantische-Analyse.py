...
# Funktionsdefinition für die späteren Zuordung der Themenbegriffe bei der Darstellung
def display_topics(model, feature_names, no_top_word, topic_idx):
    topic = model.components_[topic_idx]
    print("Top-Wörter: ") 
    print(f" ".join([feature_names[i] for i in topic.argsort()[:-no_top_words - 1:-1]]))

# Für Vergleichs- und Testzwecke können die Ergebnisse der semantischen Analysen wie folgt dargestellt werden
print('Der Nachteil der folgenden Darstellungen ist, dass jeweils der Inhalt eines einzelnen Artikels gegenüber der identifizierten Themen dargestellt wird.')
print('Die Darstellung der insgesamt häufigsten vorkommenden Themen bedarf etwas Nachbereitung.')
print('') 

l=lsa[0]
print("Die Themen dieses Artikels stimmen der LSA Analyse nach einem Grad von: ")
for i,t in enumerate(l):
    print(f"{t*100:.2f}"," zu den Top-Wörtern von Thema ",i+1," überein.")
    feature_names = tfidf_vect.get_feature_names_out()
    no_top_words = 3
    topic_i = i
    display_topics(lsa_model, feature_names, no_top_words, topic_i)
print('')

print("Die Themen des ersten Artikel passen nach LDA-Analyse: ")
for i,t in enumerate(lda_top[0]):
    print(f"{t*100:.2f}","% zu Thema von ",i+1)
    # Parameter für display_topics-Funktion definieren
    feature_names = tfidf_vect.get_feature_names_out()
    no_top_words = 1
    topic_i = i
    display_topics(lda_model, feature_names, no_top_words, topic_i)
